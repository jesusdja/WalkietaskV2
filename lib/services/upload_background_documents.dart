    ...
    try{
      ...
      if(value['status_code'] == 201){
        ...
        if(value['status_code'] == 500){
          blocIndicatorProgress.inList.add({'progressIndicator' : 1, 'viewIndicatorProgress' : true, 'erroSMS' : 'Usted no cuenta con espacio disponible para subir archivos adjuntos. Debe adquirir un plan de pago.', 'errorSpace' : true});
        }
      ...
      print(e.toString());
      print('NO CREADO - ${data[1]}');
      blocIndicatorProgress.inList.add({'progressIndicator' : 1, 'viewIndicatorProgress' : true, 'erroSMS' : 'Error al subir imagen : ${e.toString()}', 'errorSpace' : true});
      await Future.delayed(Duration(seconds: 1));
      blocIndicatorProgress.inList.add({'progressIndicator' : 1, 'viewIndicatorProgress' : true, 'cant' : (listDocuments.length - x)});
    }
